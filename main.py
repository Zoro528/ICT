...
parser = argparse.ArgumentParser(
    description='Interpolation consistency training')
...
parser.add_argument('--arch', default='cnn13', type=str,
                    help='either of cnn13, WRN28_2 , cifar_shakeshake26')
...
if args.dataset == 'cifar10':
    ...
    trainloader, validloader, unlabelledloader, testloader, num_classes = load_data_subset(
        1, args.batch_size, args.n_cpus, 'cifar10', data_source_dir, labels_per_class=args.num_labeled, valid_labels_per_class=args.num_valid_samples)
    zca_components = np.load(args.data_dir + 'zca_components.npy')
    zca_mean = np.load(args.data_dir + 'zca_mean.npy')
if args.dataset == 'svhn':
    data_source_dir = args.data_dir
    trainloader, validloader, unlabelledloader, testloader, num_classes = load_data_subset(
        1, args.batch_size, args.n_cpus, 'svhn', data_source_dir, labels_per_class=args.num_labeled, valid_labels_per_class=args.num_valid_samples)
...
def getNetwork(args, num_classes, ema=False):
    ...
    if args.arch in ['cnn13', 'WRN28_2']:
        ...
        print(
            'Error : Network should be either [cnn13/ WRN28_2 / cifar_shakeshake26')
        sys.exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluation_epochs and (epoch + 1) % args.evaluation_epochs == 0:
            ...
            if args.pseudo_label == 'single':
                ...
                filep.write(
                    "Evaluating the primary model on validation set:\n")
                prec1 = validate(validloader, model,
                                 global_step, epoch + 1, filep)
            else:
                print("Evaluating the EMA model on validation set:\n")
                filep.write("Evaluating the EMA model on validation set:\n")
                ema_prec1 = validate(validloader, ema_model,
                                     global_step, epoch + 1, filep, ema=True)
            ...
            if is_best:
                ...
                if args.pseudo_label == 'single':
                    ...
                    best_test_prec1 = validate(
                        testloader, model, global_step, epoch + 1, filep, testing=True)
                else:
                    print("Evaluating the EMA model on test set:\n")
                    filep.write("Evaluating the EMA model on test set:\n")
                    best_test_ema_prec1 = validate(
                        testloader, ema_model, global_step, epoch + 1, filep, ema=True, testing=True)
...
def train_sl(trainloader, model, optimizer, epoch, filep):
    ...
    class_criterion = nn.CrossEntropyLoss(
        reduction='sum', ignore_index=NO_LABEL).cuda()
    ...
    for i, (input, target)in enumerate(trainloader):
        ...
        assert not (np.isnan(loss.item())
                    ), 'Loss explosion: {}'.format(loss.data[0])
...
def train(trainloader, unlabelledloader, model, model2, ema_model, optimizer, epoch, filep):
    ...
    for (input, target), (u, _) in zip(cycle(trainloader), unlabelledloader):
        ...
        if args.mixup_sup_alpha:
            ...
            input_var, target_var, u_var = Variable(
                input), Variable(target), Variable(u)

            if args.mixup_hidden:
                output_mixed_l, target_a_var, target_b_var, lam = model(
                    input_var, target_var, mixup_hidden=True,  mixup_alpha=args.mixup_sup_alpha, layers_mix=args.num_mix_layer)
                lam = lam[0]
            else:
        ...
        ema_logit_unlabeled = Variable(
            ema_logit_unlabeled.detach().data, requires_grad=False)

        #class_loss = class_criterion(class_logit, target_var) / minibatch_size

        ema_class_loss = class_criterion(
            ema_logit_labeled, target_var)  # / minibatch_size
        ...
        if args.mixup_consistency:
            if args.mixup_hidden:
                ...
                output_mixed_u, target_a_var, target_b_var, lam = model(
                    u_var, ema_logit_unlabeled, mixup_hidden=True,  mixup_alpha=args.mixup_sup_alpha, layers_mix=args.num_mix_layer)
                # ema_logit_unlabeled
                lam = lam[0]
            ...
            # criterion_u(F.log_softmax(output_mixed_u,1), F.softmax(mixedup_target,1))
            mixup_consistency_loss = consistency_criterion(
                output_mixed_u, mixedup_target) / minibatch_size
        ...
        ema_prec1, ema_prec5 = accuracy(
            ema_logit_labeled.data, target_var.data, topk=(1, 5))
...
def validate(eval_loader, model, global_step, epoch, filep, ema=False, testing=False):
    ...
    for i, (input, target) in enumerate(eval_loader):
        ...
        minibatch_size = len(target_var)
        labeled_minibatch_size = target_var.data.ne(
            NO_LABEL).sum().type(torch.cuda.FloatTensor)
        assert labeled_minibatch_size > 0
...
def adjust_learning_rate(optimizer, epoch, step_in_epoch, total_steps_in_epoch):
    ...
    lr = ramps.linear_rampup(epoch, args.lr_rampup) * \
        (args.lr - args.initial_lr) + args.initial_lr
